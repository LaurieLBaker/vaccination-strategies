...
* Explore two vaccination case studies: fox rabies and covid-19
...
.left-column[
```{r, echo=FALSE, out.height=530, out.width = 680, fig.align='center', fig.cap="Animation showing distribution of vaccines."}
...
### Where and when should we drop vaccines?
...
### 1. Consecutive and geographically-coordinated vaccination is required to rapidly control and eliminate rabies.

--

`r icons::icon_style(icons::fontawesome("paw"), scale = 2)`

### 2. It is important to take into account population dynamics and  ecology of the species.
...
### 3. Policymakers need to commit to full elimination timelines and maintain comprehensive coverage to eliminate the disease.
...
iconset1 <- awesomeIconList(
   "Pharmacy" = makeAwesomeIcon(text = fontawesome("syringe"), markerColor = "orange"),
   "Health Care Center" = makeAwesomeIcon(text = fontawesome("syringe"), markerColor = "blue"),
   "Mobile Vaccination Unit" = makeAwesomeIcon(text = fontawesome("syringe"), markerColor = "red")
)
...
The people who are best placed to answer these questions and devise a strategy are the communities and healthworkers with local knowledge who are closest to the issue.
...
???
With Afrimapr I am helping to develop spatial tutorials to equip communities with the tools to engage in data science work so that they may address issues of local and/or ecocultural importance 
...
Computer science and data science can provide us with the tools to tackle issues, but

Data is always a simplification of the human experience. 
...
--
`r icons::icon_style(icons::fontawesome("wave-square", style = "solid"), scale = 2)` 
### Navigate the role of data in society with you 

--

`r icons::icon_style(icons::fontawesome("users", style = "solid"), scale = 2)` 
### Work and engage with communities around Maine
...
Dr. Thomas Muller; Dr. Conrad Freuling; Patrick Wysocki

**Afrimapr**: 

Dr. Andy South (LSTM); Dr. Ahmadou Dicko (FAO, Senegal); Anelda Van der Walt (Talarify); Mark Herringer (healthsites.io);
